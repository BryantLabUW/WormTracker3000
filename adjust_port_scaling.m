function [newPorts, pixelpercmarray] = adjust_port_scaling (xvals, yvals, ppcm, Ports)
%%  adjust_port_scaling
%
...
if sum(isnan(ppcm))>0 
    ...
    ppcm_sloped_index = find(isnan(ppcm));
    
    for i=1:size(ppcm_sloped_index,1)
        pixelpercmarray(:,ppcm_sloped_index(i))=(m*yvals(:,ppcm_sloped_index(i)))+b;
...
if contains(info.assaytype, 'C02_3.75')
    Ports.Lx = Ports.Lx - (ppcm * info.portradius); % shift left port x location to the left by the port radius in pixels
    Ports.Rx = Ports.Rx + (ppcm * info.portradius); % shift right port x location to the right by the port radius in pixels
end
